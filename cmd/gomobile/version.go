...
import (
	...
	"github.com/konglong147/newgomomas/internal/sdkpath"
)
...
func runVersion(cmd *command) (err error) {
	// Check this binary matches the version in github.com/konglong147/newgomomas/cmd/gomobile
	...
	version, err := func() (string, error) {
		...
		cmd := exec.Command("go", "list", "-f", "{{.Stale}}", "github.com/konglong147/newgomomas/cmd/gomobile")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command("go", "list", "-f", "{{.Dir}}", "github.com/konglong147/newgomomas/app").CombinedOutput()
	if err != nil {
