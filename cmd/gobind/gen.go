...
import (
	...
	"github.com/konglong147/newgomomas/bind"
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named, libName string) {
	...
	case "java":
		...
		if p == nil {
			dir, err := packageDir("github.com/konglong147/newgomomas/bind")
			if err != nil {
				errorf(`"github.com/konglong147/newgomomas/bind" is not found; run go get github.com/konglong147/newgomomas/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(dir, "..")) // github.com/konglong147/newgomomas directory.
			...
			javaDir, err := packageDir("github.com/konglong147/newgomomas/bind/java")
			if err != nil {
				errorf("unable to import bind/java: %v", err)
				return
			}
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			dir, err := packageDir("github.com/konglong147/newgomomas/bind/objc")
			if err != nil {
